...
- 프로세스의 물리적 주소 공간을 연속적이지 않게 허용하려면
  - 물리 메모리를 프레임이라고 하는 고정 크기의 블록으로 나눔
  - 논리적 메모리를 페이지라는 동일한 크기의 블록으로 나눔
...
- 이 시스테멩서 모든 데이터/instruction 액세스에는 페이지 테이블용과 데이터/instruction 용 두 개의 메모리 액세스가 필요
...
- If page size is 4KB(2<sup>12</sup>)
  ...
  - 만약 two level schem라면 내부 페이지 테이블은 2<sup>10</sup> 4-byte 엔트리
