...
- 일반적으로 프로그램을 실행하기 위해서는 프로그램의 일부만 메모리에 있으면 됨
  ...
  - 전체 프로그램이 필요한 경우에도 모든 프로그램이 동시에 필요한 것은 아님
...
- Virtual address space
  ...
  - 프로세스가 메모리에 저장되는 방식에 대한 logical view
    - 일반저긍로 주소 0에서 시작해서 공백이 끝날때까지 연속 주소
...
- Stages in Demand Paging (worse case)
  ...
  7. 디스크 I/O 서브 시스테메엇 인터럽트 수신(I/O 완료)
...
- Thrashing을방지하려면 프로세스가 필요한만큼 프레임을 제공해야 함
...
- D ≡ the total demand for frames $= \sum WSS<sub>i</sub>$
...
- 단편화(fragmentation) 없음, 빠른 메모리 요청
  ...
  ![image-20210114135359599](image-20210114135359599.png)
...
- **Prepaging**
  ...
  - prepage를 사용하지 ㅇ낳는 경우 -> I/O 및 메모리 낭비
  - s 페이지가 프리 페이징되고 페이지의 a가 사용된다고 가정
    - Cost of s*a saved page faults
    - Cost of prepaging s*(1-a) unnecessary pages
