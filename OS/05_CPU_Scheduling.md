...
- 운영체제가 프로세스를 프로세서에 할당하는 것을 디스패치(Dispatch)라고 함
...
- 비선점 스케줄링 방식
- FCFS 정책에 따른 평균 대기시간은 최소가 아니며 프로세스 CPU 버스트 시간이 크게 다를 경우 크게 달라질 수 있음
- Convoy Effect
...
- Problem
  - 우선순위가 높은 프로세스의 꾸준한 흐름은 낮은 우선순위의 프로세스가 CPU를 확보하는 것을 방지할 수 있음
...
- 다단계 피드백 대기열 스케줄링은 다음 매개변수에 의해 정의
  - 대기열 수
  - 대기열 별 스케줄링 알고리즘
  - 프로세스 업그레이드 시게이 사용되는 방법
  - 프로세스 강등시기에 사용되는 방법
...
- SMP(Symmetric Multiprocessing)
  - 각 프로세서는 자체 예약, 모든 프로세스는 공통 준비 대기열에 있거나 각 프로세서에는 자체 준비 프로세스의 개인 레디 큐가 있음
    - 현재 가장 일반적
...
- 소프트 실시간 시스템(Soft Real-Time Systems)
  - 중요한 실시간 프로세스가 언제 예약될지 보장하지 않음
- 하드 실시간 시스템(Hard Real-Time Systems)
  - 작업은 마감일까지 서비스를 받아야 함
- 두 가지 유형의 지연 시간이 성능에 영향을 미침
  1. Interrupt latency
     - 인터럽트 도착부터 서ㅓ비스가 인터럽트하는 루틴 시작까지의 시간
  2. Dispatch latency
     - 일정이 현재 프로세스를 CPU에서 제거하고 다른 프로세스로 전환하는데 걸리는 시간
...
- 실시간 스케줄링을 위해 스케줄러는 선제적 우선 순위 기반 스케줄링을 지원해야 함
  - 그러나 소프트 실시간만 보장
- 하드 리얼 타임의 경우 마감일을 맞추는 능력도 제공해야 함
- 프로세스에는 새로운 특성이 있음 : 주기정긴 프로세스에는 일정한 간격으로 CPU가 필요
  - 처리 시간 t, 기한 d, 기간 p
  - 0 <= t <= d <= p
  - 주기적인 작업 비율은 1/p
